    ...
    <div style={{minHeight:"100vh",background:"#09090B",color:"#E4E4E7",fontFamily:"'Pretendard','JetBrains Mono',-apple-system,sans-serif"}}>
      <div style={{background:"rgba(255,255,255,0.02)",borderBottom:"1px solid rgba(255,255,255,0.06)",padding:"16px",position:"sticky",top:0,zIndex:10,backdropFilter:"blur(12px)"}}>
        <div style={{maxWidth:900,margin:"0 auto",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
          <button onClick={()=>navigate("/")} aria-label="홈으로 돌아가기" style={{background:"none",border:"none",color:"#60A5FA",cursor:"pointer",fontSize:14,padding:0,display:"flex",alignItems:"center",gap:4}}>← 홈으로</button>
      ...
      <div style={{maxWidth:900,margin:"0 auto",padding:"20px 16px"}}>
        ...
        <div role="group" aria-label="카테고리 필터" style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:20}}>
          {categories.map(cat => (
            <button key={cat} onClick={()=>setFilter(cat)} aria-pressed={filter===cat} style={{padding:"6px 14px",borderRadius:20,fontSize:13,fontWeight:600,cursor:"pointer",border:"1px solid",transition:"all 0.2s",background:filter===cat?"rgba(96,165,250,0.15)":"transparent",color:filter===cat?"#60A5FA":"#71717A",borderColor:filter===cat?"rgba(96,165,250,0.3)":"rgba(255,255,255,0.08)"}}>{cat}</button>
          ))}
        </div>
